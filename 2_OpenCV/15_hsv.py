...
H: 무슨 색. 색상환 형태(원의 형태. 직선x)(0 ~ 179 각도를 변경하며 결정)
S: 얼마나 선명한 색인지(채도 0에 가까우면 투명)(0 ~ 225)
V: 얼마나 밝은가(명도 밝은지 어두운지) (0 ~ 255)
...
mask = cv2.imread("./images/mask_plane.bmp", cv2.IMREAD_GRAYSCALE) # roi(region of interest) 이미지에서 관심 있는 영역
...
# 범위 안에 있는 픽셀은 255(흰색), 범위 밖의 픽셀은 0(검정)인 마스크를 만듦
blue_mask = cv2.inRange(hsv, lower_blue, upper_blue)

airplane_only = cv2.copyTo(airplane, mask)
composite = field.copy()
cv2.copyTo(airplane, mask, composite) # 비행기만 가져와 composite에 넣음
